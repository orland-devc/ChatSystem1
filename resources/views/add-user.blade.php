        ...
        <div class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
            <div class="bg-white shadow-md rounded-lg overflow-hidden">
                <div class="bg-white flex justify-between items-center px-6 py-4 bg-gray-100 border-b border-gray-200">
                    ...
                    <a onclick="addUserTab()" class="cursor-pointer select-none pr-4 pl-8 py-2 bg-gray-800 rounded-md text-xs uppercase text-white font-semibold">
                                    ...
                                    <tbody class="text-gray-600 text-sm font-light">
                                        @foreach ($users->where('role', $role) as $user)
                                            <tr class="border-b border-gray-200 hover:bg-gray-100 cursor-pointer" onclick="window.location.href='{{ route('users.edit', $user->id) }}'">
                                                ...
                                                {{-- <td class="px-6 text-left whitespace-nowrap"><img src="{{ $user->profile_picture }}" alt="" width="30" class="m-o rounded-full"></td> --}}
                                                <td class="py-3 px-6 text-left whitespace-nowrap">
                                                    <div class="flex items-center">
                                                        @if ($user->profile_picture)
                                                            <img src="{{ asset($user->profile_picture) }}" alt="{{ $user->name }}"  width="30" class="mr-4 rounded-full border border-gray-400">
                                                        @else
                                                            <div class="h-8 w-8 text-lg rounded-full flex items-center justify-center mr-2 border text-white bg-blue-600" style="background:">
                                                        ...
                                                        @endif
                                                        @if($user->isOnline())
                                                            <ion-icon class="text-green-500" name="ellipse">
                                                        @else
                                                        <x-online-status :user="$user" />
                                                            <span class="ml-2 text-xs">active {{ $user->last_activity->diffForHumans() }}</span>
                                                        @endif                                                         
                                                    </div> 
